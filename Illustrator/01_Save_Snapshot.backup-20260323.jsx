...
/*
 * Saves the current contents of the selected objects' layers, or the active
 * layer when nothing is selected, into Snapshots > [layer name] > sN.
 */
(function () {
    ...
    try {
        ...
        var targets = resolveTargetLayers(doc, true);
        var report = [];
        var i;

        if (targets.length === 0) {
            alert("No eligible live/source layers were found from the selection or active layer.");
            return;
        }

        for (i = 0; i < targets.length; i += 1) {
            var sourceLayer = targets[i];
            var snapshotContainer = ensureChildLayer(roots.snapshots, sourceLayer.name);
            var snapshotEntry = snapshotContainer.layers.add();
            var snapshotName = getNextNumberedName(snapshotContainer, "s");
            var sourceState;
            ...
            sourceState = captureBranchState(sourceLayer);
            unlockBranchFromState(sourceState);
            copyLayerContents(sourceLayer, snapshotEntry, sourceState);
            restoreBranchState(sourceState);

            snapshotEntry.visible = false;
            snapshotEntry.locked = true;
            report.push(sourceLayer.name + " -> " + snapshotName);
        }

        alert("Saved snapshot(s):\n" + report.join("\n"));
    } catch (err) {
        alert("Save Snapshot failed: " + err);
    } finally {
