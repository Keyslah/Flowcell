...
/*
 * Restores the highest snapshot for the active layer and selected-object
 * layers without moving the current live contents into Trash.
 */
(function () {
    ...
    try {
        ...
        for (i = 0; i < targets.length; i += 1) {
            ...
            if (!latestSnapshot) {
                skipped.push(liveLayer.name + " (no snapshot found)");
                continue;
            }

            clearLayerContents(liveLayer);

            var snapshotState = captureBranchState(latestSnapshot);
            unlockBranchFromState(snapshotState);
            copyLayerContents(latestSnapshot, liveLayer, snapshotState);
            restoreBranchState(snapshotState);

            liveLayer.visible = true;
            liveLayer.locked = false;
            restored.push(liveLayer.name + " <- " + latestSnapshot.name);
        }
        ...
        var message = "Restored latest snapshot(s):\n" + restored.join("\n");
        if (skipped.length > 0) {
            message += "\n\nSkipped:\n" + skipped.join("\n");
        }
        alert(message);
    } catch (err) {
        alert("Restore Latest Snapshot failed: " + err);
    } finally {
