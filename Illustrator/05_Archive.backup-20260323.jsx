 ...
 * into Archive > [layer name] > AN, keeping Archive separate from Trash.
 */
(function () {
    ...
    try {
        ...
        for (i = 0; i < targets.length; i += 1) {
            ...
            var archiveContainer = ensureChildLayer(roots.archive, sourceLayer.name);
            var archiveEntry = archiveContainer.layers.add();
            var archiveName = getNextNumberedName(archiveContainer, "A");
            var sourceState = captureBranchState(sourceLayer);

            archiveEntry.name = archiveName;
            archiveEntry.visible = true;
            archiveEntry.locked = false;

            unlockBranchFromState(sourceState);
            moveLayerContents(sourceLayer, archiveEntry);
            restoreBranchState(sourceState);

            sourceLayer.visible = true;
            sourceLayer.locked = false;
            archived.push(sourceLayer.name + " -> " + archiveName);
        }

        if (archived.length === 0) {
            alert("Nothing was archived.\n" + skipped.join("\n"));
            return;
        }
