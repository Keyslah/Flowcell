...
function main() {
    ...
    validItems.sort(function (a, b) {
        return b.position[1] - a.position[1]; // Change the order to sort from bottom to top
    });

    // Calculate the index for the bottom 10% of objects
    var bottomTenIndex = Math.floor(validItems.length * 0.9);

    // Deselect all objects
    app.selection = null;

    // Select the bottom 10% of objects
    for (var i = bottomTenIndex; i < validItems.length; i++) {
